 ...
 * @filesource	importer/content_types/importer.content_type.members/datatype.members.php
 */
...
class Importer_content_type_members extends Importer_content_type
{
	...
	public function perform_import($settings, $data)
	{
		...
		$member_fields		= array();
		$required_fields	= array('group_id', 'username', 'screen_name',
									'email', 'unique_id', 'password');
		...
		foreach($query->result_array() as $row)
		{
			...
			{
				$required_fields[] = $row['field_name'];
			}
		}
		...
		foreach($data as $key => $entry_data)
		{
			...
			if ( $new === TRUE)
			{
				...
				{
					if ( empty($custom_data[$required_field]) && empty($member_data[$required_field]))
					{
						$missing_required =
							($missing_required === FALSE) ?
